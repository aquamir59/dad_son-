# pygame에서 button 만들고 제어하는 프로그램 참조용
...
# button 저의
class button:
    def __init__(self, text:str, position: tuple, size:tuple=(100,50), outline:bool=True):
        self.position = position
        self.size = size
        self.button = pygame.Surface(size).convert()        # 버튼 내 색 정의 위한 작업(?)
        self.button.fill((0, 0, 0))                           #  버튼 내 색 정의
        self.outline = outline

        font = pygame.font.Font(pygame.font.get_default_font(), int(0.4*self.size[1]))      # 폰트 지정, 크기 지정
...
# 버튼 갯수
number_of_buttons = 5
buttons = [button(f'button{i}', ((120 * (i % 5) + 10), ((i % 5) * 70) + 10)) for i in range(number_of_buttons)]
print(type(buttons))
while True:

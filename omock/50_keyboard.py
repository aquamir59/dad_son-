...
while running:
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_LEFT:
            ...
            keyboard_height += 50
            if keyboard_height > 941:
                keyboard_height = 941

        elif event.key == pygame.K_SPACE:
            # 키보드 포지션을 받아서 배열 위치로 변경
            array_width = int((keyboard_width - 40) / 50)
            array_height = int((keyboard_height - 40) / 50)
    ...
    screen.blit(mark_color,(keyboard_width, keyboard_height))

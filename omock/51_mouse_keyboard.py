...
# 흑돌과 백돌의 선택을 list로 입력
device = [black_var.get(),white_var.get()]
...
board_width = 490        # 키보드 사용시 처음 위치
board_height = 490   
device_change = 0           # 흑돌, 백돌 번갈아 가며 지정한 도구(마우스 or 키보드 ) 선택하기 위한 변수
...
while running:
    ...
    clock.tick(30)       # 초당 화면 새로 보여주는 횟수 5 -> 초당 5번 화면 update
    ...
    if device[device_change] == '마우스':
        ...
        if mouse_pos[0] >= 40 and mouse_pos[0] < 990 and mouse_pos[1] >= 40 and mouse_pos[1] < 990 \
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if board_array[array_height, array_width]  == 0:
                    ...
                    # 흑돌과 백돌 교체 
                    device_change = int((device_change + 1) % 2)
                    black_white = not black_white
                    doll_change(black_white)

            # 넓은 사각형 표시할 위치 계산
            board_width = (int((mouse_pos[0] + 10) / 50)) * 50 - 10
            board_height = (int((mouse_pos[1] + 10) / 50)) * 50 - 10
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                # 키보드 포지션을 받아서 배열 위치로 변경
                array_width = int((board_width - 40) / 50)
                array_height = int((board_height - 40) / 50)
                
                # 배열내 위치에 이미 다른 돌 있는지 확인
                if board_array[array_height, array_width]  == 0:
                    board_array[array_height, array_width]  = array_no
                    # 키보드에서 마우스로 변경하는 경우, 마우스 위치를 현 위치로 변경 안 하면 마크 위치가 이상해짐
                    # 그래서 마지막 위치로 오게 마우스 포인트를 현재 위치로 변경 
                    pygame.mouse.set_pos(board_width + 25, board_height + 25)
                    # 오목 여부 확인
                    result_decision = board_decision(array_height, array_width)
    ...
    screen.blit(mark_color,(board_width, board_height))
